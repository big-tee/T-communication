    ...
    <div class="col-12 ">
        <table class="text-center" width="100%"> 
            <tr style="color:green;">
                <td><h1><b>HP LAPTOP</b></h1></td>
            </tr>
        </table>
        <div class="access">
            <?php
                $hp_array = $db_handle->runQuery("SELECT * FROM laptop  WHERE company = 'hp'");
                if (!empty($hp_array)) 
                { 
                    foreach($hp_array as $key=>$value)
                    {
            ?> 
                <form class="list" method="post" action="index.php?id=<?php echo $hp_array[$key]["id"];?>" >
